...
class Client():
    def __init__(self, clientPath, serverPath):
        self._ClientPipe = clientPath
        self._ServerPipe = serverPath

    # main
    def open_connection(self):

        # create pipe if it does not already exist
        if not os.path.exists(self._ClientPipe):
            os.mkfifo(self._ClientPipe)
    ...
    # '''
        requestPipe = open(self._ServerPipe, 'w')
        # opens pipe
    ...
    # '''
        if not os.path.exists(self._ServerPipe):
...
client = Client(CLIENT_PIPE, SERVER_PIPE)
client._check_pipe()
client.open_connection()
req = input("")
client.send_request(req)
with open(CLIENT_PIPE, 'r') as response_pipe:
